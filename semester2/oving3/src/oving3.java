...
class OppgaveOversikt {
    ...
    public boolean regNyStudent(String navn){
        for(int x = 0; x < antStud; x++){
        if(studenter[x].getNavn().equals(navn)) return false;
        }
        studenter[antStud] = new Student(navn);
        if(studenter[antStud].getNavn().equals(navn)){
            antStud++;
            if(studenter.length == antStud) this.utvidTabell();
            return true;
        }
        return false;
    }

    // Utvider tabellen med 5 indekser
    private void utvidTabell() {
        Student[] hjelp = new Student[studenter.length + 5];
        for (int x = 0; x < antStud; x++) {
            hjelp[x] = studenter[x];
        }
        studenter = hjelp;
    ...
    // Søker gjennom tabellen etter gitt navn og returnerer indexen studenten ligger på. Hvis studenten ikke finnes returneres -1.
    private int getNavnIndex(String navn){
        int index = -1;
        for(int x = 0; x < antStud; x++){
            if(studenter[x] != null){
                if(studenter[x].getNavn().toUpperCase().equals(navn.toUpperCase())) {
                    index = x;
                    return index;
                }
            }
    ...
    public int finnAntOppgaver(String navn){
        int oppgaver;
        if(studenter[getNavnIndex(navn)].getAntOppg() > 0){
            oppgaver = studenter[getNavnIndex(navn)].getAntOppg();
            return oppgaver;
        } else return -1;
    }

    public boolean økAntOppg(String navn, int add){
        int index = getNavnIndex(navn);
        int oppgaver = studenter[index].setAntOppg(studenter[index].getAntOppg() + add);
        if(oppgaver == studenter[index].getAntOppg()) return true;
        return false;
    }
